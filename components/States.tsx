...
const {aspectRatio, states, screenshotSizes} = statesConfig;
...
const States = (props: Props) => {
  ...
  const statesToShow = useMemo(
    () => (maxCount ? states.slice(0, maxCount) : states),
    [maxCount],
  );
  return (
    <ItemGrid>
      {statesToShow.map(({key, sheet, name, query}) => {
      ...
      {maxCount && maxCount < states.length ? (
        <Link href="/ecosystem/apps">
          <a>
            <MoreBox>
              <Icon icon={IconNames.ARROW_RIGHT} /> &nbsp; More states
            </MoreBox>
          </a>
        </Link>
      ) : null}
