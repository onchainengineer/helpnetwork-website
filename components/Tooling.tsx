...
import toolingConfig from '../tooling.json';
import styled from '@emotion/styled';
import {Colors, Icon} from '@blueprintjs/core';
import {IconNames} from '@blueprintjs/icons';

const {aspectRatio, tooling, screenshotSizes} = toolingConfig;
...
const Ecosystem = (props: Props) => {
  ...
  const toolingToShow = useMemo(
    () => (maxCount ? tooling.slice(0, maxCount) : tooling),
    [maxCount],
  );
  return (
    <ItemGrid>
      ...
      {maxCount && maxCount < tooling.length ? (
        <Link href="/ecosystem/apps">
          <a>
            <MoreBox>
              <Icon icon={IconNames.ARROW_RIGHT} /> &nbsp; More tooling
            </MoreBox>
          </a>
        </Link>
      ) : null}
