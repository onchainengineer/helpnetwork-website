...
import valleysConfig from '../valleys.json';
import styled from '@emotion/styled';
import {Colors, Icon} from '@blueprintjs/core';
import {IconNames} from '@blueprintjs/icons';

const {aspectRatio, valleys, screenshotSizes} = valleysConfig;
...
const Valleys = (props: Props) => {
  const {maxCount} = props;
  const valleysToShow = useMemo(
    () => (maxCount ? valleys.slice(0, maxCount) : valleys),
    [maxCount],
  );
  return (
    <ItemGrid>
      ...
      {maxCount && maxCount < valleys.length ? (
        <Link href="/ecosystem/apps">
          <a>
            <MoreBox>
              <Icon icon={IconNames.ARROW_RIGHT} /> &nbsp; More valleys
            </MoreBox>
          </a>
        </Link>
      ) : null}
